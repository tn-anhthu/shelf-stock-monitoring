"""One-off script to verify the cluster_rows chaining fix
(src/pipeline/row_clustering.py) against real detected boxes on the 5 raw
calibration images, per docs/superpowers/specs/
2026-08-06-cluster-rows-chaining-fix-design.md.
...
the right tool for checking "did the gap/row-grouping fix change what I
expect" without paying for classification you haven't touched yet.
...
To compare against pre-fix behavior: check out the previous version of
row_clustering.py, run, restore, run again, diff the two outputs (see
Task 2 Step 2 of docs/superpowers/plans/2026-08-06-cluster-rows-chaining-fix.md
